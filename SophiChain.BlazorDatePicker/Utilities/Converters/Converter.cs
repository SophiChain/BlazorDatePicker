...
#nullable enable
    public class Converter<T, U>
    {
        ...
        public U? Set(T? value)
        {
            ...
            {
                SetError = true;
                SetErrorMessage = ("Conversion failed from {0} to {1}: {2}", [typeof(T).Name, typeof(U).Name, e.Message]);
            }
            return default(U);
        }
        ...
        public T? Get(U? value)
        {
            ...
            {
                GetError = true;
                GetErrorMessage = ("Conversion failed from {0} to {1}: {2}", [typeof(U).Name, typeof(T).Name, e.Message]);
            }
            return default(T);
        }
